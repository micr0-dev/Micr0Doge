...
#List of uwu's
uwuList= ["UwU", "OwO", "-w-", "UuU", "UnU", "T-T", "TwT", "qwq", "q-q", "QwQ", "-_-", "._.",".-.","^_^", "^-^", "U^U", "-o-", ";-;", ";w;", "O-O", "(｡●ω●｡)", "UpU", "UoU", "U-U", "U.U", "U,U", "TnT", "T~T", "O>O", "O-o", "O_O", "U^U", "T-T", "@-@", "@_@", "UvU", "OuO", "-c-", "-o-"]
...
█▄─▀█▀─▄█▄─▄█─▄▄▄─█▄─▄▄▀█─▄▄─█▄─▄▄▀█─▄▄─█─▄▄▄▄█▄─▄▄─█
██─█▄█─███─██─███▀██─▄─▄█─██─██─██─█─██─█─██▄─██─▄█▀█
▀▄▄▄▀▄▄▄▀▄▄▄▀▄▄▄▄▄▀▄▄▀▄▄▀▄▄▄▄▀▄▄▄▄▀▀▄▄▄▄▀▄▄▄▄▄▀▄▄▄▄▄▀ By Micr0byte, For Squabbi (Version 1.1)\n""")
...
async def on_message(message):
    ...
    for emoticon in uwuList:
        if emoticon.lower() in message.content.lower():
            await message.channel.send(uwuList[randint(0,len(uwuList)-1)])
            break
    ...
    while True:
        ...
        logmsg = "["+str(datetime.now().strftime("%m/%d/%Y, %H:%M:%S"))+"] #"+message.channel.name+" (Channel: "+str(message.channel.id)+") "+str(message.author.name)+" (User: "+str(message.author.id)+") "+": "+str(message.content)+"\n"
        print(logmsg, end='')
        ...
        if message.content[0] == "!":
            ...
            if listMessage[0] == "help":
                ...
                if version == "l":
                    ...
                    tableImageData = tableDataText(tablelist[list(codeNameDict.values()).index(codeName)])[0]
                else:
                    imgLink = zipLinksList[-1]
                    tableImageData = tableDataText(tablelist[list(codeNameDict.values()).index(codeName)])[-1]
                await message.channel.send('''```diff
- WARNING: Please use these Images only for '''+codeName+" ("+list(codeNameDict.keys())[list(codeNameDict.values()).index(codeName)].title()+"). If not it may result in a hard brick of the device -```"+'\n'+"Version: `"+str(tableImageData[0])+"` \n"+"SHA-256 Checksum: `"+str(tableImageData[-1])+"`\n"+imgLink)
            ...
            elif listMessage[0] == "download" and message.author.id == 531311619548905472:
                if len(listMessage) <= 1:
                    await message.channel.send("Invalid file. Avalible files to download: `log`, `alias`")
                    break
                if listMessage[1].lower() == "log":
                    await message.channel.send(file=discord.File(str(os.path.realpath(__file__))[:-6]+"Log.txt"))
                elif listMessage[1].lower() == "alias":
                    await message.channel.send(file=discord.File(str(os.path.realpath(__file__))[:-6]+"alias.pkl"))
                else: 
                    await message.channel.send("Invalid file. Avalible files to download: `log`, `alias`")
